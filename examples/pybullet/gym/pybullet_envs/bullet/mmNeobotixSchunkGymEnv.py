...
class MMNeobotixSchunkGymEnv(gym.Env):
    ...
    def _reset(self):
        ...
        #private variante
        self._neoschunk = mmNeobotixSchunk.MMNeobotixSchunk(urdfRootPath=self._urdfRoot, timeStep=self._timeStep)
        self._envStepCounter = 0
        p.stepSimulation()
    ...
    def getExtendedObservation(self):
        observation = self._neoschunk.getObservation()
    ...
    def _step(self, action):
        ...
        for i in range(self._actionRepeat):
            self._neoschunk.applyAction(action_scaled)
            p.stepSimulation()
